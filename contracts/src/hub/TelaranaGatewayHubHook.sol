    ...
    ReentrancyGuard
{
    ...
    function beforeSwap(
        ...
        // exact-input convention: amountSpecified < 0, "specified" side is the
        // input currency. We do NOT alter the specified side (PoolManager
        // already handles input collection via the swap call's caller-side
        // settlement). On the unspecified side we credit USDC = -amountReceived
        // (negative because the hook contributed those tokens to the pool's
        // accounting — symmetric to v4-core's BeforeSwapDelta convention used
        // by FxSwapHook).
        int128 unspecifiedDelta = -_toInt128(amountReceived);
        BeforeSwapDelta delta = toBeforeSwapDelta(int128(0), unspecifiedDelta);

        emit GatewayRoutedSwap(poolId, routeId, context.requestId, sender, context.amount, amountReceived);
