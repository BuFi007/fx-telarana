...
\title{\protocol: A Cross-Chain FX Credit Hub for Avalanche Stablecoin Markets}
\author{
...
\begin{abstract}
\protocol{} is an onchain foreign-exchange credit hub for Avalanche
stablecoin markets. The protocol lets users enter from supported chains with
USDC or EURC where Circle supports the route, move USDC between \protocol{}
hubs through Circle Gateway, route into Avalanche FX markets, and borrow, lend,
or prepare spot-FX requests against supported fiat-stablecoin pairs. Morpho
Blue provides the isolated lending substrate; a single oracle read surface,
...
state represented by repository commit \commithead{}. It is not investment,
...
\protocol{} is organized as a hub-and-spoke system. Avalanche is the primary
mainnet hub target. Fuji and Arc Testnet are testnet hubs. Other chains can
act as spokes or intent origins, depending on the asset and route.
...
\begin{tabular}{llll}
\toprule
Asset & Role & Mainnet status in design & Notes \\
\midrule
USDC & Settlement and collateral anchor & Supported & Circle rail asset. \\
AUDF & AUD stablecoin & Avalanche target & Six decimals in reference data. \\
JPYC & JPY stablecoin & Avalanche target & Mainnet decimals differ from some test deployments. \\
MXNB & MXN stablecoin & Avalanche target & Six decimals in reference data. \\
KRW1 & KRW stablecoin & Avalanche target & Decimals must be verified before mocks. \\
ZCHF & CHF stablecoin & Avalanche target & CCIP/issuer route considerations. \\
...
Avalanche stablecoin markets, then expand toward hub-to-hub USDC liquidity,
non-USDC approved asset routes, private Ghost Mode execution, RFQ Pasillo, and
future trading frontends without relaxing the core collateral-safety model.
...
We thank \texttt{@criptopoeta} on X for the protocol direction, requirements,
market references, architecture pressure, and product framing. We thank the
...
by the maintainer, with \texttt{@criptopoeta} on X as co-author and protocol
owner.
...
Human co-author & \texttt{@criptopoeta} on X \\
Repository & \url{https://github.com/BuFi007/fx-telarana} \\
Current source commit & \commithead{} \\
